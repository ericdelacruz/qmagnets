...
if(!empty($submit))
{
 ...
 $sql_ins="INSERT INTO `services` set `content`='".$body."',`title`='".$title."',`sub_cat_id`='".$category."',`image`='".$image."'"; 
	$res_ins=mysql_query($sql_ins);	
	?>
     <script language="javascript">
        window.location.href='manage_services.php?retcode=1&page=<?php echo $page; ?>';
  </script>
    <?php
   ...
   <?php
	 ...
	 $sql_ins="update `services` set				  	
				  `content`= '".$body."',
				  `title`='".$title."',
				  `image`='".$image."',
				  `sub_cat_id`='".$category."'
				   where serv_id='".$about_id."'"; 
   ...
   <?php
$sql_show="select * from services where serv_id='".$about_id."' ";
    ...
    <div style="clear:both;"></div>
		...
		<div class="status success" id="success"  onclick="cls()">
		 ...
		 ?>
              ...
              <input type="hidden" name="about_id" value="<?php echo $row_user['serv_id']; ?>" />
              <input type="hidden" name="page" value="<?php echo $page; ?>" />
            <div class="contentbox">
            	    <div id="msg_error" style="color:#FF0000; font-weight:bold;">
                       <h2><?php echo $msg; ?></h2>
                    </div>
            		<?php $q_name=mysql_query("select * from sub_category where category_id='2'");   ?>
				<p>
                	<label for="textfield"><strong>Page Name:</strong></label>
                	<select name="category">
                    <?php 
					while($f_name=mysql_fetch_array($q_name)) { 
							$subcatid=$f_name['sub_cat_id'];
					 ?>
                    <option  value="<?php echo $f_name['sub_cat_id']; ?>"<?php if($f_name['sub_cat_id']==$row_user['sub_cat_id']){?> selected="selected"<?php } ?>><?php echo $f_name['subcategory']; ?></option>
                    <?php }?>
                   </select>
