...
if(!empty($submit))
{
  ...
  $sql_ins="INSERT INTO `product_partner` set `partner`='".$title."',`prod_cat_id`='".$category."'"; 
	$res_ins=mysql_query($sql_ins);	
	?>
     <script language="javascript">
        window.location.href='manage_product_partners.php?retcode=1&page=<?php echo $page; ?>';
  </script>
    <?php
   ...
   <?php
	 ...
	 $sql_ins="update `product_partner` set				  	
   ...
   <?php
$sql_show="select * from product_partner where ppid='".$about_id."' ";
    ...
    <div style="clear:both;"></div>
		...
		<div class="status success" id="success"  onclick="cls()">
		 ...
		 ?>
              ...
              <input type="hidden" name="about_id" value="<?php echo $row_user['ppid']; ?>" />
              <input type="hidden" name="page" value="<?php echo $page; ?>" />
            <div class="contentbox">
            	    <div id="msg_error" style="color:#FF0000; font-weight:bold;">
                       <h2><?php echo $msg; ?></h2>
                    </div>
            		<?php $q_prodcat=mysql_query("select * from product_category ");   ?>
				<p>
                	<label for="textfield"><strong>Product Category:</strong></label>
                	<select name="category">
                    <?php 
					while($f_product=mysql_fetch_array($q_prodcat)) { 
							$catid=$f_product['prod_cat_id'];
					 ?>
                    <option  value="<?php echo $f_product['prod_cat_id']; ?>"<?php if($f_product['prod_cat_id']==$row_user['prod_cat_id']){?> selected="selected"<?php } ?>><?php echo $f_product['category']; ?></option>
                    <?php }?>
                   </select>
					...
					<p>
                        <label for="textfield"><strong>Product Partners:</strong></label>
                      <input type="text" name="title" id="title" class="inputbox" value="<?php echo $row_user['partner']; ?>" />
					
                    </p>
                <input type="submit"  class="btn" name="submit" value="Save"/> 
				<a href="manage_mission.php"><input type="cancel"  class="btnalt" name="cancel" value="Cancel"/></a>
