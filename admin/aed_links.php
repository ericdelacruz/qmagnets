...
//$metatitle=isset($_REQUEST['metatitle'])?addslashes($_REQUEST['metatitle']):"";
//$metakey=isset($_REQUEST['metakey'])?$_REQUEST['metakey']:"";
//$metadesc=isset($_REQUEST['metadesc'])?addslashes($_REQUEST['metadesc']):"";
//$shortdesc=isset($_REQUEST['shortdesc'])?addslashes($_REQUEST['shortdesc']):"";
$title=isset($_REQUEST['title'])?addslashes($_REQUEST['title']):"";
$link=isset($_REQUEST['link'])?addslashes($_REQUEST['link']):"";
$caption=isset($_REQUEST['caption'])?addslashes($_REQUEST['caption']):"";
...
if(!empty($submit))
{
    ...
    if($mode=='add')
     {  	
   //print_r($_POST); exit;
   $sql_ins="INSERT INTO `links` set `title`='".$title."',`content`='".$body."',`link`='".$link."',`caption`='".$caption."'"; 
...
<script language="javascript">
        window.location.href='manage_links.php?retcode=1&page=<?php echo $page; ?>';
       </script>
<?php if($mode=='edit')
	 {
		
	 $sql_ins="update `links` set
				  	
				  `content`= '".$body."',
				  `title`= '".$title."',
				  `link`='".$link."',
				  `caption`='".$caption."'
				   where linksid='".$about_id."'";  
		$res_ins=mysql_query($sql_ins);	
...
<?php
    ...
    ?>
<?php
$sql_show="select * from links where linksid='".$about_id."' ";
...
<script language="javascript" type="text/javascript">
 ...
 {  
  if(document.getElementById('title').value=='')
    {
	  document.getElementById('msg_error').innerHTML="Please enter link title.";
   ...
   if(document.getElementById('link').value=='')
    {
	  document.getElementById('msg_error').innerHTML="Please enter link.";
...
function spaceKillerbyRajib(that) 
{
...
<div id="rightside">
  ...
  <div class="contentcontainer">
    <div class="headings altheading">
      <h2>About Add / Edit <span style="padding-right:100px;"><a href="../" style="text-decoration:none; border:0px;" target="_blank"><img src="img/icons/preview.png" alt="Edit" height="25" width="90" border="0" /></a></span></h2>
    </div>
    <div style="clear:both;"></div>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" name="application_form" onSubmit="return validate_product('<?php echo $mode ?>');" enctype="multipart/form-data">
      ...
      <input type="hidden" name="about_id" value="<?php echo $row_user['linksid']; ?>" />
      ...
      <div class="contentbox">
        ...
        <p>
          <label for="textfield"><strong>Link Title:</strong></label>
        ...
        <p>
          ...
          <input type="text" name="link" id="link"   value="<?php echo $row_user['link']; ?>"    class="inputbox" onInput="spaceKillerbyRajib(this);
"/>
        </p>
        <p>
        
          <label for="textfield"><strong> Caption:</strong></label>
          <textarea  name="caption" id="caption" rows="15" cols="70" style="width: 50%" ><?php echo $row_user['caption'];;?></textarea>
          <br />
        </p>
        <br/>
        <p>
        ...
        <a href="manage_links.php">
        <input type="cancel"  class="btnalt" name="cancel" value="Cancel"/>
        </a> </div>
    </form>
  </div>
  <!-- Content Box End -->
